...
public class InMemoryRepository<T>
    ...
    public InMemoryRepository(List<T> data) => Data = data;

    public Task<IEnumerable<T>> GetAllAsync() => Task.FromResult(Data.AsEnumerable());

    public Task<T> GetByIdAsync(Guid id) => Task.FromResult(Data.FirstOrDefault(x => x.Id == id));

    public Task<IEnumerable<T>> GetRangeByIdsAsync(List<Guid> ids) => Task.FromResult(Data.Where(x => ids.Contains(x.Id)).AsEnumerable());
